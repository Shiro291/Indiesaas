...
export class IpaymuService {
    ...
    constructor() {
        ...
        if (!this.apiKey || !this.merchantCode) {
            throw new Error(
                "IPAYMU_API_KEY and IPAYMU_MERCHANT_CODE must be set in environment variables"
            )
        }
    }
    ...
    async createTransaction(
        ...
        try {
            const body = {
                ...
                notifyUrl:
                    request.notifyUrl ||
                    `${process.env.NEXT_PUBLIC_APP_URL}/api/payments/ipaymu-callback`
            }

            // Generate signature using the correct Ipaymu format
            const signature = this.generateSignature(
                "/api/transaksi/merchant",
                "POST",
                body,
                this.apiKey
            )
            ...
            const response = await fetch(
                `${this.baseUrl}/api/transaksi/merchant`,
                {
                    method: "POST",
                    headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    `Ipaymu API error: ${response.status} - ${errorText}`
                )
            }
            ...
            throw new Error(
                `Failed to create Ipaymu transaction: ${error.message}`
            )
        }
    }
    ...
    verifyCallbackSignature(
        ...
        try {
            ...
            const reconstructedSign = this.generateSignature(
                "/api/transaksi/notify",
                "POST",
                callbackData,
                this.apiKey
            )
            ...
            return (
                callbackData.sign === reconstructedSign &&
                callbackData.status === expectedStatus
            )
        } catch (error) {
