...
import { events, registrations, attendees } from "@/database/schema"
...
export class AnalyticsService {
    ...
    async getDashboardAnalytics(
        ...
        // Get KPIs
        const [totalEvents, totalRegistrations, revenueResult, totalAttendees] =
            await Promise.all([
                // Total events
                db
                ...
                db
                    .select({
                        sum: sql<number>`sum(${registrations.totalAmount})`
                    })
                    .from(registrations)
                    .where(
                        and(
                ...
                db
                    ...
                    .where(
                        and(
                            startDate
                                ? gte(attendees.createdAt, startDate)
                                : undefined,
                            endDate
                                ? lte(attendees.createdAt, endDate)
                                : undefined
                        )
                    )
                    .then((result) => result[0]?.count || 0)
    ...
    private async getRegistrationsReport(
        period: AnalyticsPeriod
    ): Promise<AnalyticsReport> {
        const { startDate, endDate } = this.getDateRange(period)
    ...
    private async getAttendeesReport(
        period: AnalyticsPeriod
    ): Promise<AnalyticsReport> {
        const { startDate, endDate } = this.getDateRange(period)
        ...
        const attendeeData = await db
            ...
            .innerJoin(
                registrations,
                eq(attendees.registrationId, registrations.id)
            )
            .innerJoin(events, eq(registrations.eventId, events.id))
            .where(
                and(
