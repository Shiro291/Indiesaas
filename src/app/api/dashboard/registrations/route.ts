...
export async function GET(request: NextRequest) {
  try {
    ...
    const userRegistrations = await registrationService.getRegistrationsByUserId(session.user.id);

    return Response.json({ registrations: userRegistrations });
  } catch (error: any) {
    console.error("Error fetching user registrations:", error);
    return Response.json({ error: error.message || "Failed to fetch user registrations" }, { status: 500 });
  }
}