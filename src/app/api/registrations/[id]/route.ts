...
export async function GET(
  ...
  try {
    ...
    if (registration.userId !== session.user.id) {
      return Response.json({ error: "Unauthorized to access this registration" }, { status: 403 });
    }

    return Response.json({ registration });
  } catch (error: any) {
    console.error("Error fetching registration details:", error);
    return Response.json({ error: error.message || "Failed to fetch registration details" }, { status: 500 });
  }
}