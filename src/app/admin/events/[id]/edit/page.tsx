...
export default function AdminEventFormPage({ params }: { params: { id?: string } }) {
  ...
  const [registrationOpenDate, setRegistrationOpenDate] = useState<Date | undefined>(undefined);
  const [registrationCloseDate, setRegistrationCloseDate] = useState<Date | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (isEdit) {
        ...
        setDescription("Annual championship for Pencak Silat practitioners of all levels");
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className={cn(
                          "w-full justify-start text-left font-normal",
                          !registrationOpenDate && "text-muted-foreground"
                        )}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {registrationOpenDate ? format(registrationOpenDate, "PPP") : <span>Pick a date</span>}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0">
                      <Calendar
                        ...
                        onSelect={setRegistrationOpenDate}
                        initialFocus
                      />
                    </PopoverContent>
                  </Popover>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className={cn(
                          "w-full justify-start text-left font-normal",
                          !registrationCloseDate && "text-muted-foreground"
                        )}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {registrationCloseDate ? format(registrationCloseDate, "PPP") : <span>Pick a date</span>}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0">
                      <Calendar
                        ...
                        onSelect={setRegistrationCloseDate}
                        initialFocus
                      />
                    </PopoverContent>
                  </Popover>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setAdminFee(Number(e.target.value) * 100)} // Convert to cents
                ...
                <Card key={index} className="mb-4">
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <Label htmlFor={`ticket-price-${index}`}>Price (IDR)</Label>
                        <Input
                          id={`ticket-price-${index}`}
                          type="number"
                          value={ticket.price / 100} // Convert from cents
                          onChange={(e) => handleTicketChange(index, 'price', Number(e.target.value) * 100)} // Convert to cents
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button
                              ...
                              className={cn(
                                "w-full justify-start text-left font-normal",
                                !ticket.availableFrom && "text-muted-foreground"
                              )}
                            >
                              <CalendarIcon className="mr-2 h-4 w-4" />
                              {ticket.availableFrom ? format(ticket.availableFrom, "PPP") : <span>Pick a date</span>}
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto p-0">
                            <Calendar
                              mode="single"
                              selected={ticket.availableFrom}
                              onSelect={(date) => handleTicketChange(index, 'availableFrom', date)}
                              initialFocus
                            />
                          </PopoverContent>
                        </Popover>
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button
                              ...
                              className={cn(
                                "w-full justify-start text-left font-normal",
                                !ticket.availableUntil && "text-muted-foreground"
                              )}
                            >
                              <CalendarIcon className="mr-2 h-4 w-4" />
                              {ticket.availableUntil ? format(ticket.availableUntil, "PPP") : <span>Pick a date</span>}
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto p-0">
                            <Calendar
                              mode="single"
                              selected={ticket.availableUntil}
                              onSelect={(date) => handleTicketChange(index, 'availableUntil', date)}
                              initialFocus
                            />
                          </PopoverContent>
                        </Popover>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor={`ticket-capacity-${index}`}>Max Capacity</Label>
                        <Input
                          id={`ticket-capacity-${index}`}
                          type="number"
                          value={ticket.maxCapacity}
                          onChange={(e) => handleTicketChange(index, 'maxCapacity', Number(e.target.value))}
                          placeholder="Max number of tickets"
                        />
                      </div>

                      <div>
                        <Label htmlFor={`ticket-type-${index}`}>Ticket Type</Label>
                        <Select 
                          value={ticket.type} 
                          onValueChange={(value: "ONLINE" | "ONSITE") => handleTicketChange(index, 'type', value)}
                        >
                          <SelectTrigger>
